...
async def test_formation_flight_loop_properly_spaced():
    ...
    # We are at 40.0, -75.0 which means we are at distance 0 from anchor.
    # Formation V, spacing 5.0 -> Target offset is (-5.0, 5.0).
    # Since there are no other drones, repulsion shouldn't change anything.
    peer_self = PeerStateManager("drone1")
    peer_self.last_seen = now
    peer_self.is_active = True
    peer_self.lat = 40.0
    ...
    from DroneOS.core.formation_manager import FormationManager, convert_local_offset_to_global
...
async def test_formation_flight_loop_repulsion():
    ...
    # drone2 is dangerously close to us (e.g. at our exact position, creating a push away)
    # Let's put drone2 1 meter North of us. We should be repelled South.
    ...
    # Unmodified target
    from DroneOS.core.formation_manager import FormationManager
    form_mgr = FormationManager()
    form_mgr.set_formation(FormationType.V, 5.0)
    # drone1 is index 1
